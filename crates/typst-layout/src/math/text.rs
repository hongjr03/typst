...
fn layout_inline_text(
    ...
    if text.chars().all(|c| c.is_ascii_digit() || c == '.') {
        ...
        // differently to normal text below and is worth re-evaluating in the
        // future.
        let mut fragments = vec![];
        ...
        for unstyled_c in text.chars() {
            ...
            if is_single {
                // Duplicate what `layout_symbol` does exactly even if it's
                // probably incorrect.
...
/// The non-dotless version of a dotless character that can be used by the dtls
/// OpenType feature.
